...
byte[] buffer, int size, IntPtr lpNumberOfBytesWritten);
    ...
    /// <summary>
    /// Jmp по относительным кординатам
    /// </summary>
    public void JMP(int addre) {
        this.Asmcode = this.Asmcode + "E9" + intTohex(addre, 8);
    }
    #endregion


    public void AddBytes(byte[] numArray) {
        this.Asmcode = this.Asmcode + BitConverter.ToString(numArray).Replace("-", "");
    }
